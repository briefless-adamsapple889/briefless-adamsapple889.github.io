import { Box, Choice, Group, OneOrMore, Optional, Seq } from "./diagram.js";
...
function buildRepeat(node) {
    ...
    if (lazy)
        label += " · 최소";
    if (min === 0)
        return new Optional(new OneOrMore(inner, label));
    return new OneOrMore(inner, label);
}
function anchorToken(kind) {
    if (kind === "start")
        return "^";
    if (kind === "end")
        return "$";
...
function groupClass(kind) {
    if (kind === "capture")
        return "rr-g-cap";
    if (kind.includes("look"))
        return "rr-g-look";
    return "rr-g-plain";
}
function groupCaption(node) {
    switch (node.kind) {
        case "capture":
...
function classBox(node) {
    const inside = node.parts
        .map((p) => p.kind === "range" ? `${p.from}-${p.to}` : p.kind === "escape" ? p.raw : p.value)
        .join("");
